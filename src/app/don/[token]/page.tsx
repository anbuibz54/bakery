...
export const metadata: Metadata = { title: 'Đơn bánh', robots: { index: false } }
                ...
                return (
                  <li key={s.status} className="grid grid-cols-[48px_minmax(0,1fr)] gap-3">
                    <div className="flex flex-col items-center">
                      <div
                        className={`flex size-12 items-center justify-center rounded-full ${done ? 'bg-surface' : 'bg-line/70'} ${now ? 'shadow-[inset_0_0_0_2px_var(--berry)]' : ''}`}
                      >
                        <StampIcon name={s.icon} size={30} print={done ? s.print : '#E5D8DF'} className={done ? '' : 'text-muted/70'} />
                      </div>
                      {i < steps.length - 1 && <div className={`my-1 w-0.5 flex-1 ${i < current ? 'bg-pink' : 'bg-line'}`} />}
                    </div>
                    <div className="pb-[18px]">
                      <div className="flex items-baseline justify-between gap-2">
                        <span className={`font-bold ${done ? '' : 'text-muted/80'}`}>{s.title(order)}</span>
                        {event && <span className="text-xs whitespace-nowrap text-muted">{stamp(event.createdAt)}</span>}
                      </div>
                      <p className={`text-sm ${done ? 'text-foreground/80' : 'text-muted/70'}`}>{event?.message ?? s.hint}</p>
                    </div>
                  </li>
                )
              })}
        ...
        {!order.hidePrice && order.status !== 'cancelled' && !unpaid && remaining > 0 && (
          <div className="mt-3.5 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-3xl bg-sky p-4">
            <div>
              ...
              <p className="text-[13px] text-foreground/75">tiền mặt hoặc chuyển khoản</p>
            </div>
