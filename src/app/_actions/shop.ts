...
export async function saveOccasionAction(_prev: OccasionState, form: FormData): Promise<OccasionState> {
  try {
    ...
    return { ok: true, message: `Đã lưu ngày ${saved.day}/${saved.month} của ${saved.personName}. Trước 7 ngày tiệm nhắn Zalo cho bạn.` }
  } catch (error) {
...
export async function placeOrderAction(_prev: CheckoutState, form: FormData): Promise<CheckoutState> {
  ...
  try {
    ...
    return { error: 'Tiệm chưa nhận được đơn do lỗi hệ thống. Bạn thử lại, hoặc nhắn Zalo tiệm nhé.' }
  }
